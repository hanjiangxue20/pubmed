...
def pubmed_spider():
    ...
    try:
        ...
        logger.info(json.dumps(article_list))
        if download_flag:
            logger.info('准备获取下载链接...')
            for article in article_list:
                if article['is_free_resources']:
                    try:
                        response = session.get(host + '/{}'.format(article['PMID']), headers=headers, verify=False,)
                        html = etree.HTML(response.text)
                        file_download_page = html.xpath('//span[@class="identifier pmc"]/a/@href')[0]
                        response_download_page = session.get(file_download_page, headers=headers, verify=False)
            ...
            logger.info('获取下载链接结束，开始下载{}个文献...'.format(file_queue.qsize()))
